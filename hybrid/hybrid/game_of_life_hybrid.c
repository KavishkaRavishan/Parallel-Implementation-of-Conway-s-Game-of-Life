 ...
 * Course: EC7207
 * Group: 32
 * Description: Conway's Game of Life - Hybrid (MPI + OpenMP) Implementation
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
// Function to initialize the grid reproducibly across MPI ranks
...
void print_grid(unsigned char *grid, int rows, int cols, int gen) {
    ...
    printf("Generation: %d | Live Cells: %d (Hybrid MPI+OpenMP)          \n", gen, live_count);
...
// Function to compute the next generation with OpenMP
        ...
        for (int j = 0; j < cols; j++) {
            ...
            for (int di = -1; di <= 1; di++) {
                for (int dj = -1; dj <= 1; dj++) {
                    ...
                    int ni = i + di;
                    int nj = (j + dj + cols) % cols; // Wrap for columns
...
int main(int argc, char *argv[]) {
    ...
    // Count local live cells using OpenMP reduction
    int local_count = 0;
